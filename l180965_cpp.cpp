...
void printAccounts(bankAccount ** x);
...

{
	...
	*/
	...
	b2[0]=new bankAccount (30,2000,trans1,3);
	...
	*/
	...
	b3[0]=new bankAccount (50,2000,trans,2);
	...
	*/
		int size=0;
		bankAccount ** d4 = readFile("input.txt");		
...
bankAccount** readFile(string filename)
{
	float *transactionarr;
	ifstream in;
	in.open(filename);
	int accounts=0,id=0,no_of_trans=0;
	float balance =0,balance_limit=0;
	in>>accounts;
	bankAccount **dyarr=new bankAccount*[accounts];
	for(int i=0;i<accounts;i++)
	{
		...
		in>>balance_limit;
		in>>no_of_trans;
		transactionarr=new float[no_of_trans];
		for(int i=0;i<no_of_trans;i++)
		{
			in>>transactionarr[i];
		
		}
		*dyarr=new bankAccount(id,balance,transactionarr,no_of_trans,balance_limit);
	}
	in.close();
...
bool makeTransaction(bankAccount **x,){}
//void deleteAccounts(....){}
